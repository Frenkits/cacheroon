    ...
    entry.addEventListener("click", () => {
      ...
      if (report) {
        ...
        details.innerHTML = `
          ...
          <strong>Creato:</strong> ${created}<br>
          <strong>Rimosso:</strong> ${deleted}<br>
          <strong>Via:</strong> ${report.street || "Sconosciuta"}<br>
...
async function addReport(lat, lng, description = null) {
  ...
  addChat(`✅ Cacca segnalata il ${new Date(report.created_at).toLocaleString()} in ${report.street || "Sconosciuta"}`, report.id);
  activeCount++;
  updateStats();
...
async function deleteReport(id) {
  ...
  addChat(`❌ Cacca rimossa il ${new Date().toLocaleString()} in ${allReports[id].street || "Sconosciuta"}`, id);
