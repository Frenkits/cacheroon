...
async function loadStats(){
  ...
  const { count: deletedCountResult } = await supabase
    .from('reports')
    .select('*', { count: 'exact' })
    .not('deleted_at', 'is', null);

  activeCount = activeCountResult || 0;
  deletedCount = deletedCountResult || 0;
  updateStats();
}
loadStats();
...
async function addReport(lat, lng, description){
  ...
  const formatted = new Date(report.created_at).toLocaleString()
  ...
  addChat(`✅ Cacca aggiunta il ${formatted} in ${report.street}`, report.id)
...
async function deleteReport(id){
  ...
  if(data){
    ...
    addChat(`❌ Cacca rimossa il ${formatted} in ${report.street}`, id)
  ...
  .on('postgres_changes', { event: '*', schema: 'public', table: 'reports' }, payload => {
    ...
    if(payload.eventType === 'UPDATE' && payload.new.deleted_at){
      ...
      if(data){
        clusterGroup.removeLayer(data.marker)
        delete markers[id]
        addChat(`❌ Cacca rimossa il ${new Date().toLocaleString()} in ${data.street}`, id)
        allReports[id].deleted_at = payload.new.deleted_at
        activeCount--
        deletedCount++
        updateStats()
      }
